...
<div class="page-user">
	{#if data.user}
		...
		<div class="score">
			<p class="type-title-lg">{data.user.score}</p>
		...
		<div class="chart">
			<div class="bar">
				{#each data.recordsAndVerifications as record}
			...
			<div class="legends">
				{#each Array(legendCount) as _, i}
					{@const x = (i * (legendCount / (legendCount - 1)) * (listLength - 1)) / legendCount}
					<div
						class="rank"
		...
		<div class="about tile">
			...
			<dd class="type-body-lg">
				...
				labore et dolore magna aliqua. duis at.
			</dd>
		</div>
		<div class="hardest tile">
			<dt class="type-title-sm">Best Achievement</dt>
			<dd class="type-body-lg">{data.user.records[0].level.name}</dd>
		</div>
