<script lang="ts">
	import { embed, ordinal } from '$lib/util';
	import type { PageData } from './$types';

	import iconYouTube from '$lib/assets/icons/media/youtube.svg';
...
<div class="page-level">
	<div class="details">
		<span class="rank type-title-lg">{String(data.rank).padStart(3, '0')}</span>
		<h2 class="name">{data.level.name}</h2>
		<iframe
			class="video"
			src={embed(data.level.video)}
		...
		<dl class="credits">
			<div class="publisher">
				...
				<dd class="type-body-lg">{data.level.user.name}</dd>
			</div>
			<div class="verifier">
				<dt class="type-title-sm">Verifier</dt>
				<dd class="type-body-lg">{data.level.verifier.name}</dd>
			</div>
			<div class="creators">
				<dt class="type-title-sm">Creators</dt>
				<dd class="type-body-lg">
					{#each data.level.creators as creator, i}
						<a href={`/users/${creator.id}`}>{creator.name}</a
						>{#if i < data.level.creators.length - 1}<span>, </span>{/if}
					{/each}
				</dd>
			</div>
		</dl>
